  ...
  async doOrder(params, type) {
    ...
    if (!flag) {
      ...
      if (!phone || phone === '0') {
        ...
        let code = new Array(14)
        chooseInfo.map(item => {
          let choose = item.spfchoose.join('')
        ...
        codes = code.join(',')
        if (params.chooseLength > 9) {
          codes = codes + ':1:5'
        } else {
          codes = codes + ':1:1'
        }
        let gid = betConfig.r9.gid
